...
class Client(object):
    ...
    def configure(self, config, inverter):
        logging.getLogger("mqtt").setLevel(config.get('log_level', logging.INFO))
        logging.info(f"Configuring MQTT client...")
        ...
        if self.config['homeassistant']:
            for ha_sensor in config['mqtt'].get('ha_sensors'):
        ...
        logging.info(f"MQTT client configured successfully. Host: {self.config['host']}, Port: {self.config['port']}, HA Discovery: {self.config['homeassistant']}")
        return True
    ...
    def on_connect(self, client, userdata, flags, reason_code, properties):
        ...
        if reason_code > 0:
            logging.warning(f"MQTT: FAILED to connect {client._host}:{client._port}")

    def on_disconnect(self, client, userdata, flags, reason_code, properties):
        if reason_code == 0:
            logging.info(f"MQTT: Server Disconnected")
        if reason_code > 0:
            logging.warning(f"MQTT: FAILED to disconnect {reason_code}")
    ...
    def on_message(self, client, userdata, msg):
        ...
        logging.debug(f"MQTT: Received message on topic {topic} with payload {payload}")
        if topic.startswith(self.config['topic'] + "/set/"):
            register_name = topic.replace(self.config['topic'] + "/set/", "")
            logging.debug(f"MQTT: Set command received for register {register_name} with value {payload}")
            # Find the register linked to this topic
            for reg in self.ha_sensors:
                if reg.get('register', None) == register_name:
                    if reg.get('mode', None) and 'w' in reg.get('mode'):
                        logging.info(f"MQTT: Setting value for register {register_name} to {payload}")
                        reg['last_set_value'] = payload
                    else:
                        logging.warning(f"MQTT: Received set command for register {register_name} but it is not writable")
                    break
            else:                
                logging.warning(f"MQTT: Received set command for register {register_name} but it was not found in the configuration")
    ...
    def publish(self, inverter):
        ...
        if self.config['homeassistant'] and not self.ha_discovery_published:
            ...
            for ha_sensor in self.ha_sensors:
                ...
                config_msg['state_topic'] = self.config['topic']
        ...
        payload = json.dumps(inverter.inverter_config | inverter.client_config | inverter.last_scrape).replace('"', '\"')
        logging.debug(f"MQTT: Publishing Registers: {self.config['topic']} : {payload}")
        self.mqtt_queue.append(self.mqtt_client.publish(self.config['topic'], payload, qos=0).mid)
