...
class Client(object):
        ...
        if self.config['homeassistant']:
            ...
            for ha_sensor in self.ha_sensors:
                ...
                if reg_name and not inverter.validateRegister(reg_name):
                    ...
                    return False
                valid_sensors.append(ha_sensor)
            self.ha_sensors = valid_sensors
    ...
    def on_message(self, client, userdata, msg):
        ...
        if topic.startswith(self.config['topic'] + "/set/"):
            ...
            log.debug(f"MQTT: Set command received for register {register_name} with value {payload}")
            ...
            for reg in self.ha_sensors:
                if reg.get('register', None) == register_name:
                    if reg.get('mode', None) and 'w' in reg.get('mode'):
                        log.info(f"MQTT: Setting value for register {register_name} to {payload}")
                        reg['last_set_value'] = payload
                    else:
                        log.warning(f"MQTT: Received set command for register {register_name} but it is not writable")
                    break
            else:                
                log.warning(f"MQTT: Received set command for register {register_name} but it was not found in the configuration")
    ...
    def publish(self, inverter):
        ...
        if self.config['homeassistant'] and not self.ha_discovery_published:
            ...
            for ha_sensor in self.ha_sensors:
                ...
                log.debug(f'MQTT: Topic; {ha_topic}, Message: {config_msg}')
        ...
        log.debug(f"MQTT: Publishing Registers: {self.config['topic']} : {payload}")
        self.mqtt_queue.append(self.mqtt_client.publish(self.config['topic'], payload, qos=0).mid)
        log.info(f"MQTT: Registers Published")
